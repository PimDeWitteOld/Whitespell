...
function HistoryController() {
    ...
    HistoryController.Instance.prototype = {
        ...
        restorePage: function (data) {
            ...
            var toRestore = updates[updates.length -2 ];
    ...
    initialize: function () {
        ...
        try {
            window.addEventListener("popstate", function (e) {
                whitespell.history.restorePage(e.state);
            }, false);
        } catch (er) {
        ...
        whitespell.workers.assign("xmr", {
            ...
            try {
                ...
                whitespell.workers.assign("xmr", {
                    ...
                    whitespell.history.setStartState(toPut);
...
var retrieveVariables = function (input, javascripted, parentElement) {
    ...
    return retrieveVariables(input, javascripted);
}
    ...
    Layout.Instance.prototype = {
        ...
        updatePage: function(data) {
          ...
          for(var i in data.components) {
              ...
              document.getElementById(data.components[i]).innerHTML = new Component(type, data.updates[i], data.components[i]).update; //whitespell.parseUrl(whitespell.config.updates[fillInName].remote_html)
          }
        ...
        updateComponentContent: function (component, fillInName, animation) {
            if (whitespell.config.components[component].history === true) {
                whitespell.history.pushState({updateName: fillInName, componentName: component}, "", "#" + fillInName);
            }

            var type = "update";

            if(component === fillInName) {
                type = "main";
            }
            document.getElementById(component).innerHTML = new Component(type, fillInName, component).update; //whitespell.parseUrl(whitespell.config.updates[fillInName].remote_html)
        },

        loadComponent: function (name) {

            structure[name] = {
                content: new Component("main", name).update, //whitespell.parseUrl(whitespell.config.components[name].remote_html),
                location: whitespell.config.components[name].position
            }
           /* if (whitespell.config.components[name].history === true) {
                whitespell.history.pushState({componentName: name}, "", "#" + name);
            }*/
        },
        ...
        initialize: function () {
            ...
            //structure["footer"] = new Footer().update;

            for (var comps in structure) {
                document.getElementById(structure[comps].location).innerHTML += structure[comps].content;
            }
        ...
        addScriptsToDom: function (name, content) {
...
function Component(updateType, componentName, mainComponent) {
    ...
    var content = "<div id=\"" + componentName + "\"></div>";
    ...
    function notifyReceival() {
        if (collects.html_content.length > 0
            && collects.js_content.length > 0
            && collects.css_content.length > 0
            ) {
            if (!constructedPage) {
    ...
    function constructPage() {
        ...
        if (style.styleSheet) {
            // IE
            style.styleSheet.cssText = collects.css_content;
        } else {
            // Other browsers
            style.innerHTML = collects.css_content;
        }

        document.getElementsByTagName("head")[0].appendChild(style);

        var z = function (data, compName) {
            ...
            if (javascripted) {
                whitespell.layout.addScriptsToDom(compName, collects.js_content);
            }
        };

        whitespell.history.setOnPageLoad(componentName, collects.html_content, z);

        z(collects.html_content, componentName);
    }
